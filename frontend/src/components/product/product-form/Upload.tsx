          ...
          <div className={styles['image-list']}>
            <label htmlFor='add-image'>
              <input
                ...
                accept='image/*'
                hidden
                disabled={product.value.images.length >= 6}
              />
              <AddMedia images={product.value.images} />
            </label>
            <ul>
              {product.value.images.map((image, index) => (
                <li key={index}>
                  <img
                    src={image instanceof File? URL?.createObjectURL(image): typeof image === 'string'? image: ''}
                    alt={image?.name}
                  />
                  <button
...
const UploadVideo = (props) => {
  ...
  useEffect(() => {
    console.log({ videoThumb: product.value.videoThumbnailUrl });
  }, []);

  useEffect(() => {
    // console.log({ thumbnailUrl });
    console.log({ finalUrl: product.value.videoThumbnailUrl? product.value.videoThumbnailUrl: thumbnailUrl? thumbnailUrl: '' });
  ...
  return (
    <>
      ...
      {(product.value.video || product.value.videoThumbnailUrl) && (
